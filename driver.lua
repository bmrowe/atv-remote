...
do	--Globals
	...
	init = "fresh"
	oldDashboardInfo = {}
	SWITCH_DEVICE = SWITCH_DEVICE or nil
	PYATV_RUNTIME = PYATV_RUNTIME or {
		ws_connected = false,
		app_list_ready = false,
		startup_until = 0,
		last_device_selected_ms = 0,
...
function PYATV.MarkStartupWindow (seconds)
	seconds = tonumber(seconds) or 8
	local untilMs = PYATV.NowMs() + (seconds * 1000)
	if (untilMs > (PYATV_RUNTIME.startup_until or 0)) then
		PYATV_RUNTIME.startup_until = untilMs
	end
	dbg ("Startup window active for "..seconds.."s")
end
...
function PYATV.ConnectionState(state)
    ...
    if (state) then
	   state = "connected"
	   PYATV_RUNTIME.ws_connected = true
	   PYATV_RUNTIME.last_ws_connected_ms = PYATV.NowMs()
    else
	   state = "disconnected"
	   PYATV_RUNTIME.ws_connected = false
    end
...
function PYATV.ConnectDevice () -- ORIGINAL: connect_device
	...
	PYATV.MarkStartupWindow(8)
	if (PYATV_RUNTIME.last_connected_device_id ~= tostring(id or '')) then
		PYATV_RUNTIME.app_list_ready = false
		for k,_ in pairs(APP_LIST) do APP_LIST[k] = nil end
		PYATV_RUNTIME.last_connected_device_id = tostring(id or '')
	end
...
function PYATV.GetAppList (callback)
	dbg ("---Get App List---")
	...
	C4:urlGet(url, {}, false,
		function(ticketId, strData, responseCode, tHeaders, strError)
			if (strError == nil) then
				...
				dbg (strData)
				array = JSON:decode(strData) or {}
				local apps = array["Apps"] or {}
				if (next(apps) == nil) then
					dbg ("App list response did not contain Apps entries.")
				...
				PYATV_RUNTIME.app_list_ready = true
...
function PYATV.LaunchApp (id)
	if (not id or id == '') then
		print ("Launch app skipped: missing app ID")
		return
	end
	local now = PYATV.NowMs()
	if (PYATV_RUNTIME.last_launch_app_id == id) and ((now - (PYATV_RUNTIME.last_launch_app_ms or 0)) < 1500) then
		dbg ("Skipping duplicate Launch_App for "..id)
		return
	end
	...
	local delays = {0}
	if (inStartupWindow and not (PYATV_RUNTIME.ws_connected and PYATV_RUNTIME.app_list_ready)) then
		delays = {0, 1800, 3600}
	elseif (PYATV_RUNTIME.current_app_id ~= '' and PYATV_RUNTIME.current_app_id ~= id) then
		-- Fast follow-up helps when first launch command is accepted but tvOS is slow to transition apps.
		delays = {0, 600}
	end
			...
			function(ticketId, strData, responseCode, tHeaders, strError)
				if (strError == nil) then
					...
					if (responseCode ~= 200) then
						print ("Launch app returned response code "..tostring(responseCode).." for app "..id)
					else
						dbg ("Launch app command accepted for app "..id)
					end
				else
					print("C4:urlGet() failed: "..strError)
				end
	...
	for i, delay in ipairs(delays) do
		...
		local delayMs = delay
		if (delay <= 0) then
			LaunchAttempt(attempt, #delays)
		else
			C4:SetTimer(delayMs, function(timer)
				LaunchAttempt(attempt, #delays)
			end)
		end
	end
...
function OnDriverLateInit ()
    ...
    RegisterRooms()
	OnPropertyChanged ('Device to control')
end
...
function OnPropertyChanged (strProperty)
        local deviceValue = Properties [strProperty]

	if (strProperty == 'Device to control') then
		SWITCH_DEVICE = deviceValue
	end
...
function ReceivedFromProxy (idBinding, strCommand, tParams)
	...
	local function FuzzyFindAppId(sourceTable, targetName)
		local targetNorm = NormalizeAppName(targetName)
		if (not targetNorm or targetNorm == "") then
			return nil, nil
		end
		for candidateName, candidateId in pairs(sourceTable or {}) do
			if (NormalizeAppName(candidateName) == targetNorm) then
				return candidateId, candidateName
			end
		end
		if (string.len(targetNorm) >= 4) then
			for candidateName, candidateId in pairs(sourceTable or {}) do
				local candidateNorm = NormalizeAppName(candidateName)
				if (candidateNorm and (string.find(candidateNorm, targetNorm, 1, true) or string.find(targetNorm, candidateNorm, 1, true))) then
					return candidateId, candidateName
				end
			end
		end
	...
	local function ResolveLaunchAppId(appId, appName)
		if (appId and appId ~= '') then
			if (string.find(tostring(appId), '%.')) then
				dbg ("Miniapp provided App ID: "..appId)
				return appId
			end
			-- Some miniapps pass a friendly app name in APP_ID; treat that as a name to resolve.
			dbg ("Miniapp APP_ID appears to be app name: "..tostring(appId))
			if (not appName or appName == '') then
				appName = tostring(appId)
			end
		end
		if (appName and APP_LIST[appName]) then
			dbg ("PyATV Dynamic App ID found: "..APP_LIST[appName])
			return APP_LIST[appName]
		end
		if (appName and APPLE_TV[appName]) then
			dbg ("Apple TV Pre-defined App ID found: "..APPLE_TV[appName])
			return APPLE_TV[appName]
		end
		local fuzzyDynamicId, fuzzyDynamicName = FuzzyFindAppId(APP_LIST, appName)
		if (fuzzyDynamicId) then
			dbg ("PyATV Fuzzy App ID found: "..fuzzyDynamicId.." (matched "..tostring(appName).." -> "..tostring(fuzzyDynamicName)..")")
			return fuzzyDynamicId
		end
		local fuzzyPredefinedId, fuzzyPredefinedName = FuzzyFindAppId(APPLE_TV, appName)
		if (fuzzyPredefinedId) then
			dbg ("Apple TV Fuzzy App ID found: "..fuzzyPredefinedId.." (matched "..tostring(appName).." -> "..tostring(fuzzyPredefinedName)..")")
			return fuzzyPredefinedId
		end
		return nil
	end
	local args = {}
	...
	if (idBinding == MSP_PROXY) then
		if (strCommand == 'DEVICE_SELECTED') then
			local nowMs = PYATV.NowMs()
			if ((nowMs - (PYATV_RUNTIME.last_device_selected_ms or 0)) < 8000) then
				dbg ("Skipping duplicate DEVICE_SELECTED within debounce window")
				MSP.ON (idBinding, strCommand, tParams, args)
				return
			end
			PYATV_RUNTIME.last_device_selected_ms = nowMs
			PYATV.pollMediaInfo ("proxy")
			local wsFresh = PYATV_RUNTIME.ws_connected and ((nowMs - (PYATV_RUNTIME.last_ws_connected_ms or 0)) < 30000)
			if (wsFresh) then
				dbg ("Skipping reconnect on DEVICE_SELECTED (existing websocket is fresh)")
				if (not PYATV_RUNTIME.app_list_ready) then
					PYATV.GetAppList()
				end
			else
				PYATV.MarkStartupWindow(8)
	...
	if (idBinding == SWITCHER_PROXY) then
		if (strCommand == 'SET_INPUT') then
			...
			if (not input) then
				print ("Invalid SET_INPUT request: INPUT is nil or non-numeric")
				return
			end
			if (input >= MINIAPP_BINDING_START and input <= MINIAPP_BINDING_END) then
				...
				if (not proxyDeviceId) then
					print ("Unable to resolve switcher proxy device ID.")
					return
				end
				...
				if (not appProxyId) then
					print ("No app proxy found for input "..tostring(input))
					return
				end
				...
				if (not appDeviceId) then
					print ("No app protocol device found for proxy "..tostring(appProxyId))
					return
				end
				...
				local resolvedAppId = ResolveLaunchAppId(appId, appName)
				if (not resolvedAppId) then
					print ("No App ID found for Application: "..tostring(appName).." (refreshing app list and retrying once)")
					PYATV.GetAppList(function(success)
						local retryAppId = ResolveLaunchAppId(appId, appName)
						if (retryAppId) then
							ExecuteCommand ('Launch_App', { ['App'] = retryAppId })
						elseif (success) then
							print ("No App ID found for Application after refresh: "..tostring(appName))
						end
					end)
					return
				end
				ExecuteCommand ('Launch_App', { ['App'] = resolvedAppId }) -- utilize existing mini app drivers without modifications.
					...
					local _timer = function (timer)
						...
						for roomId, deviceId in pairs (RoomIDSources) do
							if (deviceId == appProxyId) then
								C4:SendToDevice (roomId, 'SELECT_VIDEO_DEVICE', {deviceid = SWITCH_DEVICE})
							end
						end
					end
